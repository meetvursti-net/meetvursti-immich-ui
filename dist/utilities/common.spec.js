import { isEnabled, isExternalLink, resolveMetadata, resolveUrl } from './common.js';
...
describe(resolveMetadata.name, () => {
    ...
    it('should handle page metadata', () => {
        expect(resolveMetadata({ title: 'Site title', description: 'Site description' }, { title: 'Page title', description: 'Page description' })).toEqual({
            type: 'website',
    ...
    it('should handle page metadata with an image', () => {
        expect(resolveMetadata({ title: 'Title', description: 'Description' }, { title: 'Title', description: 'Description', imageUrl: 'page-image' })).toMatchObject({
            imageUrl: 'page-image',
        });
    });
    it('should prefer the page image', () => {
        expect(resolveMetadata({ title: 'Title', description: 'Description', imageUrl: 'site-image' }, { title: 'Title', description: 'Description', imageUrl: 'page-image' })).toMatchObject({
            imageUrl: 'page-image',
        });
    });
    it('should detect an article', () => {
        expect(resolveMetadata({ title: 'Title', description: 'Description' }, { title: 'Article title', description: 'Article description' }, { publishedTime: DateTime.fromObject({ year: 2025, month: 1, day: 1 }) })).toMatchObject({
            type: 'article',
        });
    });
});
